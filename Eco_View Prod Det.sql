...
select (SELECT (concat(TO_CHAR(LPAD(j.nrocgc,12,'0')),TO_CHAR(LPAD(j.digcgc,2,'0')))) FROM CONSINCO.GE_EMPRESA J WHERE J.NROEMPRESA = X.NROEMPRESA) as CNPJ,
       ...
       case
         when Y.pesavel = 'S' then 'KG' else 'UN' end as unit,
