SELECT NROTITULO||'/'||X.NROPARCELA TITULO_PARC, C.SEQPESSOA||' - '||C.NOMERAZAO FORNECEDOR, NROEMPRESA EMPRESA, CODESPECIE, 
  ...
  FROM CONSINCO.FI_TITULO X LEFT JOIN CONSINCO.GE_PESSOA C ON X.SEQPESSOA = C.SEQPESSOA
                            LEFT JOIN CONSINCO.FI_TITOPERACAO Z ON X.SEQTITULO = Z.SEQTITULO AND  Z.OPCANCELADA IS NULL AND Z.CODOPERACAO IN 
                            (180,181,182,191,194,197,203,229)
                            
  WHERE (UPPER(X.OBSERVACAO) LIKE '%BAIX%' AND UPPER(X.OBSERVACAO) LIKE '%OP%' AND UPPER(X.OBSERVACAO) NOT LIKE '%DEVOL%' OR UPPER(X.OBSERVACAO) LIKE '%SOMENTE BAIXA%')
                              AND X.SITUACAO != 'C'
                              AND DTAVENCIMENTO BETWEEN :DT1 AND :DT2
                              AND NROEMPRESA IN (#LS1)
AND CODESPECIE NOT IN ('ACRCOM', 'DUPPCO')
