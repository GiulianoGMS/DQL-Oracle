...
SELECT LA.NROEMPRESA EMP_CTB, TO_CHAR(LA.DTALANCTO, 'DD/MM/YYYY') DATA,
       LA.TIPOLANCTO TP,
       CASE WHEN LA.CODOPERACAO = 452 THEN 11100200101 ELSE CASE WHEN LA.TIPOLANCTO IN ('D', 'H', 'C') THEN 
          CASE WHEN LA.TIPOLANCTO = 'D' THEN DB.CONTADB
               WHEN LA.TIPOLANCTO = 'C' THEN DT.CONTADB END ELSE DB.CONTACR END END PARTIDA,  
       CASE WHEN LA.CODOPERACAO = 452 THEN 11200100004 ELSE CASE WHEN LA.TIPOLANCTO IN ('D', 'H', 'C') THEN 
         CASE WHEN LA.TIPOLANCTO = 'H' THEN DB.CONTADB 
           ...
           C_PARTIDA,
       CASE WHEN ROW_NUMBER() OVER(PARTITION BY LA.NROPROCESSO, LA.CODOPERACAO ORDER BY LA.NROPROCESSO ASC,3, O.CODOPERACAO) = 1 OR LA.CODOPERACAO IN (134,122,123) THEN
  ...
  FROM FI_CTACORLANCA LA LEFT JOIN FI_CONTABILIZACAO A ON A.SEQLANCTO = LA.SEQLANCTO AND LA.NROEMPRESA = A.NROEMPRESA AND A.NROPROCESSO = LA.NROPROCESSO AND DEBCRED = LA.TIPOLANCTO
                        INNER JOIN FI_CTACORRENTE CTA  ON CTA.SEQCTACORRENTE = LA.SEQCTACORRENTE
                         LEFT JOIN FI_TITOPERACAO O    ON O.NROPROCESSO = LA.NROPROCESSO AND LA.CODOPERACAO != 452 AND LA.CODOPERACAO NOT IN (122,123) AND LA.TIPOLANCTO != 'H' AND NVL(O.OPCANCELADA, 'X') != 'C'
