...
FROM MAP_PRODUTO A
       ...
       LEFT JOIN (SELECT DISTINCT MR.SEQPRODUTO, MAX(MR.DTAULTVENDA) DTAES FROM MRL_PRODUTOEMPRESA MR GROUP BY SEQPRODUTO) EPP
       ON EPP.SEQPRODUTO = EP.SEQPRODUTO

WHERE A.DTAHORINCLUSAO BETWEEN :DT1 AND :DT2
      AND EP.STATUSCOMPRA != 'I' 
      AND EPP.DTAES < SYSDATE - 180
   OR A.DTAHORINCLUSAO BETWEEN :DT1 AND :DT2
      AND EP.STATUSCOMPRA != 'I' 
      AND EPP.DTAES IS NULL
...
GROUP BY A.SEQPRODUTO, B.CODACESSO , A.DESCCOMPLETA, 
               ...
               TO_CHAR(II.DTAULTCOMPRA,'DD-MM-YYYY'), MARCA, NOMERAZAO, DTAES,
