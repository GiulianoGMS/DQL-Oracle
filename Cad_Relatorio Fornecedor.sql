...
select distinct (d.seqproduto),
                ...
                CASE WHEN F.QTDEMBALAGEM = 1 THEN (SELECT SUM(AA.ESTOQUE) FROM MRL_PRODLOCAL AA, MRL_LOCAL BB, MRL_PRODUTOEMPRESA CC
                     WHERE AA.SEQLOCAL = BB.SEQLOCAL
                     AND AA.SEQPRODUTO = D.SEQPRODUTO
                     AND BB.TIPLOCAL = 'L' AND aA.nroempresa = bB.nroempresa and cC.seqproduto = Aa.seqproduto and cC.nroempresa = Aa.nroempresa) ELSE NULL END ESTOQUE_LOJA,
                CASE WHEN F.QTDEMBALAGEM = 1 THEN (SELECT SUM(AA.ESTOQUE) FROM MRL_PRODLOCAL AA, MRL_LOCAL BB, MRL_PRODUTOEMPRESA CC
                     WHERE AA.SEQLOCAL = BB.SEQLOCAL
                     AND AA.SEQPRODUTO = D.SEQPRODUTO
                     AND BB.TIPLOCAL = 'T' AND aA.nroempresa = bB.nroempresa and cC.seqproduto = Aa.seqproduto and cC.nroempresa = Aa.nroempresa) ELSE NULL END ESTOQUE_AVARIA,    
                CASE WHEN F.QTDEMBALAGEM = 1 THEN (SELECT SUM(AA.ESTOQUE) FROM MRL_PRODLOCAL AA, MRL_LOCAL BB, MRL_PRODUTOEMPRESA CC
                     WHERE AA.SEQLOCAL = BB.SEQLOCAL
                     AND AA.SEQPRODUTO = D.SEQPRODUTO
                     AND BB.TIPLOCAL = 'O' AND aA.nroempresa = bB.nroempresa and cC.seqproduto = Aa.seqproduto and cC.nroempresa = Aa.nroempresa) ELSE NULL END ESTOQUE_OUTROS,    
                CASE WHEN F.QTDEMBALAGEM = 1 THEN (SELECT SUM(AA.ESTOQUE) FROM MRL_PRODLOCAL AA, MRL_LOCAL BB, MRL_PRODUTOEMPRESA CC
                     WHERE AA.SEQLOCAL = BB.SEQLOCAL
                     AND AA.SEQPRODUTO = D.SEQPRODUTO
                     AND BB.TIPLOCAL = 'D' AND aA.nroempresa = bB.nroempresa and cC.seqproduto = Aa.seqproduto and cC.nroempresa = Aa.nroempresa) ELSE NULL END ESTOQUE_CD
  ...
  where a.seqfamilia = b.seqfamilia 
   ...
   -nd NVL(y.descricao,'OUTROS') in (#LS2)  
