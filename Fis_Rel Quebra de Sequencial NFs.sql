...
(WITH FUROS AS
 (SELECT NROEMPRESA, SERIE, NUM_ANT + 1 AS INICIO_FURO, NUMERO - 1 AS FIM_FURO
   ...
   WHERE NUMERO - NUM_ANT > 1
     AND NOT EXISTS (SELECT 2 FROM MLF_NFINUTILIZACAO I WHERE I.NROEMPRESA = XX.NROEMPRESA AND I.NRINICIALNF = XX.NUM_ANT + 1 AND I.SERIENF = XX.SERIE))
...
SELECT CASE WHEN LENGTH('#LS1') > 10 THEN 'Todas' ELSE '#LS1' END  NROEMPRESA, NULL SERIE, 'Sem Furos!' NF_FALTANTE
  FROM DUAL
 WHERE NOT EXISTS (SELECT 1 FROM FUROS)) X WHERE 1=1
