...
/*Criado por Giuliano em 16/11/2023*/
  ...
  UPDATE CONSINCO.MRL_PROMOCAOITEM PI
       SET PI.DTAINICIOPROM = (SELECT CASE WHEN :DT1 > P.DTAFIM OR :DT1 >= DTAINICIO OR :DT1 < TRUNC(SYSDATE)
                                      THEN DTAINICIO ELSE :DT1 END FROM CONSINCO.MRL_PROMOCAO P WHERE P.SEQPROMOCAO = :NR1),
    ...
    UPDATE CONSINCO.MRL_PROMOCAO P
       SET P.DTAINICIO          = (SELECT CASE WHEN :DT1 > P.DTAFIM OR :DT1 >= DTAINICIO OR :DT1 < TRUNC(SYSDATE)
                                      THEN DTAINICIO ELSE :DT1 END FROM CONSINCO.MRL_PROMOCAO P WHERE P.SEQPROMOCAO = :NR1),
...
SELECT CASE WHEN TO_DATE(DTAINICIO, 'DD/MM/YY') = :DT1 THEN 'Data Alterada Com Sucesso!' 
...
SELECT CASE WHEN TO_DATE(DTAINICIO, 'DD/MM/YY') = :DT1 THEN 'Promoção: '||A.PROMOCAO||' - Data inicio: '||TO_DATE(DTAINICIO, 'DD/MM/YYYY')
