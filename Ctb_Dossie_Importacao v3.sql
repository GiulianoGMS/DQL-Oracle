-- QRP em https://github.com/GiulianoGMS/QRPs/blob/main/RelDossieImportacao_v3.QRP
...
SELECT XX1.*,
 /*Select de Subtototais*/
  ...
  FROM FI_TITULO X 
 WHERE X.NROTITULO    IN (#LT2) AND NROEMPRESA IN (502,503) AND SITUACAO != 'C' AND CODESPECIE = 'ANTPGI') TOT_TIT_GER,
 /******/
      (SELECT 'R$ '||TO_CHAR(SUM(VLRORIGINAL * DECODE(OBRIGDIREITO, 'D',-1,'O',1)),  'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''')
  FROM FI_TITULO X 
 WHERE X.NRODOCUMENTO = #NR2  AND NROEMPRESA IN (502,503) AND SITUACAO != 'C') TOT_DESP_GER,
 /******/
...
'R$ '||TO_CHAR(((SELECT SUM(VLRORIGINAL)
 ...
 WHERE X.NROTITULO    IN (#LT2) AND NROEMPRESA IN (502,503) AND SITUACAO != 'C')  -
 /******/
      (SELECT SUM(VLRORIGINAL * DECODE(OBRIGDIREITO, 'D',-1,'O',1))
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA   
 WHERE X.NRODOCUMENTO = #NR2  AND NROEMPRESA IN (502,503) AND SITUACAO != 'C')  -
 /******/
 ...
 WHERE ZZ.DTAEMISSAO > SYSDATE - 365
  ...
  (SELECT NVL(SUM(VLRLANCAMENTO),0) FROM FI_CTACORLANCA F
   WHERE 1=1 
     AND UPPER(HISTORICO) LIKE '%TÍTULO(S):%'||SUBSTR('#LT2',INSTR('#LT2',',') +1,10)||'%'
     AND TIPOLANCTO = 'C'
...
SELECT * FROM (
 SELECT 'TITULO' TIPO, CODESPECIE, SEQTITULO, NROEMPRESA EMP, X.SEQPESSOA||' - '||NOMERAZAO FORNECEDOR, NROTITULO, #NR2 PROCESSO,
  ...
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA
 WHERE X.NROTITULO IN (#LT2) AND NROEMPRESA IN (502,503) AND SITUACAO != 'C' AND X.OBRIGDIREITO = 'D'  ORDER BY 9)
...
SELECT 'TITULO', NULL ,NULL, NULL, 'TOTAL DE TITULOS GERADOS', NULL, NULL, 
  ...
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA
 WHERE X.NROTITULO IN (#LT2) AND NROEMPRESA IN (502,503) AND SITUACAO != 'C' AND CODESPECIE = 'ANTPGI'
...
SELECT 'ORCAMENTO' TIPO, CG.CODESPECIE CODESPECIE,
       ...
       'OBR' OD,
       DECODE(CG.SITUACAO, 'A', 'ABERTO' , 'QUITADO') SITUACAO, 3
