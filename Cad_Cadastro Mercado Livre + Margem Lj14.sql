SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/ A.SEQPRODUTO SKU, 'Predeterminado' DIVISAO, DESCCOMPLETA DESC_PROD,
      ...
      (SELECT ROUND(NVL(NULLIF(MGMPRECOVDAPROMOC,0),MGMPRECOVALIDO),2) FROM MAXV_MGMBASEPRODSEG X WHERE X.SEQPRODUTO = A.SEQPRODUTO AND X.NROEMPRESA = S.NROEMPRESA AND X.NROSEGMENTO = SE.NROSEGMENTO AND X.QTDEMBALAGEM = SE.QTDEMBALAGEM) MARGEM,
       CASE WHEN NVL(SE.PRECOGERPROMOC,0) > 0 THEN SE.PRECOGERPROMOC ELSE SE.PRECOGERNORMAL END PRECO_VENDA, 1 MIN_COMPRA, 1 MULT_COMPRA,
  ...
  FROM MAP_PRODUTO A INNER JOIN MAP_FAMDIVISAO B ON A.SEQFAMILIA = B.SEQFAMILIA
                     ...
                     INNER JOIN MRL_PRODEMPSEG SE ON SE.SEQPRODUTO = A.SEQPRODUTO AND SE.NROEMPRESA = 14 AND NROSEGMENTO = 2 AND SE.QTDEMBALAGEM = H.QTDEMBALAGEM
 ...
 WHERE S.STATUSCOMPRA = 'A' AND CASE WHEN NVL(SE.PRECOGERPROMOC,0) > 0 THEN SE.PRECOGERPROMOC ELSE SE.PRECOGERNORMAL END > 0
