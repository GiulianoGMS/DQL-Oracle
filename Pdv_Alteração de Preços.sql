...
SELECT DISTINCT PLU, BB.CODACESSO EAN, DESCCOMPLETA DESCRICAO, AA.QTDEMBALAGEM, PRECO_ANTERIOR, PRECO_NORMAL, PRECO_PROMOCIONAL, PRECOATUAL, DATA_ALTERACAO, DATA_INICIO_PROMO, DATA_FIM_PROMO, 
...
SELECT DISTINCT A.SEQPRODUTO PLU,
       ...
       TO_CHAR(NVL(( SELECT MAX(P.DTAINCLUSAO)
         FROM    CONSINCO.MRLV_PROMOCAOITEM P
         WHERE   P.SEQPRODUTO = D.SEQPRODUTO
         AND     D.NROEMPRESA        =           P.NROEMPRESA
         AND     D.NROSEGMENTO       =           P.NROSEGMENTO
         AND     D.QTDEMBALAGEM      =           P.QTDEMBALAGEM
         AND     NVL(TRUNC(H.DTAPROGALTERACAO), TRUNC(D.DTAGERACAOPRECO))   BETWEEN P.DTAINICIO AND P.DTAFIM
       ),A.DTAHORALTERACAO), 'DD/MM/YYYY HH24:MI') DATA_INCLUSAO,
...
AND    A.NROSEGMENTO IN (SELECT X.NROSEGMENTOPRINC FROM MAX_EMPRESA X WHERE X.NROEMPRESA IN (#LS1)) 
