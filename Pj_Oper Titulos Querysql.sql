...
SELECT FORNECEDOR, CASE WHEN TO_CHAR(DOC_C5) = TIT_C5 THEN TO_CHAR(DOC_C5) ELSE DOC_C5||' - Tit.: '||TIT_C5 END DOC_C5, 
...
SELECT 
       ...
       CASE WHEN (F.VLRORIGINAL - (FC.VLRDESCCONTRATO + F.VLRPAGO)) != (DDA2.VALORDOCUMENTO - (DDA2.VALORDESCONTO1 + DDA2.VALORABATIMENTO)) 
         THEN 'Valor Total Sistema: '||TO_CHAR((F.VLRORIGINAL - (FC.VLRDESCCONTRATO + F.VLRPAGO)),'FM999G999G999D90', 'nls_numeric_characters='',.''')||' - DDA: '||
              TO_CHAR((DDA2.VALORCOMDESCONTO - DDA2.VALORABATIMENTO),'FM999G999G999D90', 'nls_numeric_characters='',.''')||' | Desconto Sistema: '||
              TO_CHAR((FC.VLRDESCCONTRATO+FC.VLRDSCFINANC+F.VLRPAGO),'FM999G999G999D90', 'nls_numeric_characters='',.''')||' - DDA: '||
              TO_CHAR((DDA2.VALORDESCONTO1 + DDA2.VALORDESCONTO2+DDA2.VALORDESCONTO3+DDA2.VALORABATIMENTO),'FM999G999G999D90', 'nls_numeric_characters='',.''')
