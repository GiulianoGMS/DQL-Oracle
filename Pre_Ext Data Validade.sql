...
SELECT X.NROEMPRESA, INITCAP(G.NOMERAZAO) FORNEC, 
       ...
       TO_CHAR(ROUND(M.CMULTVLRNF,2), 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''')  CUSTO,
       CASE WHEN X.STATUS = 'A' THEN TO_CHAR(X.VLRPRECOPROMOC, 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''')  ELSE NULL END VLR_REBAIXA, 
       TO_CHAR(S.PRECOVALIDNORMAL, 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''')  PRECO_NORMAL,
       X.QTDESOLICITADA,
  ...
  FROM MRL_PROMOCESPECIALHIST X INNER JOIN MAP_PRODUTO P     ON P.SEQPRODUTO    = X.SEQPRODUTO
                                ...
                                INNER JOIN MRL_PRODEMPSEG S  ON S.SEQPRODUTO    = X.SEQPRODUTO AND S.NROEMPRESA = X.NROEMPRESA AND S.NROSEGMENTO = E.NROSEGMENTOPRINC
