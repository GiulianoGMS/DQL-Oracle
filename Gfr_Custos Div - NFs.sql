 ...
 SELECT DISTINCT X.NROEMPRESA, X.SEQPESSOA||' - '||G.NOMERAZAO FORNECEDOR, C.COMPRADOR, X.NUMERONF,
        SUM(REPLACE(REGEXP_SUBSTR(B.DESCRICAO,'[0-9]+(\.[0-9]+)?'),'.',',') -
            REPLACE(REGEXP_SUBSTR(SUBSTR(B.DESCRICAO,100),'[0-9]+(\.[0-9]+)?'),'.',',')) VLR_DIF,
         
        B.USUAUTORIZOU USU_AUTORIZACAO, TO_CHAR(B.DTAAUTORIZOU, 'DD/MM/YYYY HH24:MI') DTA_AUTORIZACAO
 ...
 WHERE 1=1
   ...
   GROUP BY X.NROEMPRESA, X.NUMERONF, B.USUAUTORIZOU, TO_CHAR(B.DTAAUTORIZOU, 'DD/MM/YYYY HH24:MI'), X.SEQPESSOA, G.NOMERAZAO, C.COMPRADOR
