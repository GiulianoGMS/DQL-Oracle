SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/ X.NROEMPRESA LOJA, X.SEQPRODUTO PLU, DESCCOMPLETA DESCRICAO_PRODUTO, COMPRADOR,
 ...
 WHERE X.NROEMPRESA   = #LS1
   ...
   AND ROUND(ESTQLOJA / F.PADRAOEMBCOMPRA,2) >= :NR1
   AND (CATEGORIAN1 IN (:LS2) OR NVL(:LS2, 'X') = 'X')
   ...
   GROUP BY X.SEQPRODUTO, DESCCOMPLETA, COMPRADOR,
       ...
       TO_CHAR(NAGF_INICIOPROMETIQUETA(X.NROEMPRESA, S.SEQPRODUTO, S.PRECOVALIDPROMOC), 'DD/MM/YYYY'),
       TO_CHAR(NAGF_FIMPROMETIQUETA(X.NROEMPRESA, S.SEQPRODUTO, S.PRECOVALIDPROMOC), 'DD/MM/YYYY')
