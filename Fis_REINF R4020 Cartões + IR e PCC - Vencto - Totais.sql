...
SELECT X.NROEMPRESA LOJA,
       ...
       TO_CHAR(SUM(X.VALOR_NF_COMISSAO), 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''') VALOR_NF,
       TO_CHAR(SUM(X.VALOR_PARC), 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''') VALOR_PARC,
       TO_CHAR(CASE WHEN TO_DATE(VENC_IRRF, 'DD/MM/YYYY') BETWEEN :DT1 AND :DT2 THEN SUM(X.IRRF)       ELSE NULL END, 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''') IR,
       CASE WHEN TO_DATE(VENC_IRRF, 'DD/MM/YYYY') BETWEEN :DT1 AND :DT2 THEN X.VENC_IRRF  ELSE NULL END VENC_IR,
       TO_CHAR(CASE WHEN TO_DATE(VENC_PCC,  'DD/MM/YYYY') BETWEEN :DT1 AND :DT2 THEN SUM(X.TOTAL_PCC)  ELSE NULL END, 'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''') PCC,
...
FROM CONSINCO.NAGV_ITWORKS_REINF_IR_PCC X
 ...
 WHERE (TO_DATE(VENC_IRRF, 'DD/MM/YYYY') BETWEEN :DT1 AND :DT2 OR TO_DATE(VENC_PCC, 'DD/MM/YYYY') BETWEEN :DT1 AND :DT2)
   ...
   GROUP BY X.NROEMPRESA, X.CNPJ_EMP, DTAENTRADA, DTAVENCIMENTO, NOMERAZAO, COD_NAT_REND, VENC_IRRF, VENC_PCC
...
SELECT NULL, NULL, NULL, NULL, 'TOTAL', NULL, 
