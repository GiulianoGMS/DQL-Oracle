SELECT X.NROEMPRESA LOJA, X.SEQPRODUTO PLU, P.DESCCOMPLETA, MARCA,
       ...
       COMPRADOR, G.SEQPESSOA COD_FORNEC, G.NOMERAZAO FORNECEDOR
  ...
  FROM MRL_PRODUTOEMPRESA X INNER JOIN MAP_PRODUTO P ON P.SEQPRODUTO = X.SEQPRODUTO
                            ...
                            INNER JOIN MAX_COMPRADOR C ON C.SEQCOMPRADOR = FD.SEQCOMPRADOR
                            INNER JOIN MAP_FAMFORNEC FF ON FF.SEQFAMILIA = P.SEQFAMILIA AND PRINCIPAL = 'S'
                            INNER JOIN GE_PESSOA G ON G.SEQPESSOA = FF.SEQFORNECEDOR
                             
 WHERE NROEMPRESA = 56
   AND STATUSCOMPRA = 'A'

GROUP BY X.NROEMPRESA, X.SEQPRODUTO, P.DESCCOMPLETA, DECODE(STATUSCOMPRA, 'A', 'Ativo', 'S', 'Suspenso', 'I', 'Inativo'),
      CATEGORIAN1, CATEGORIAN2, CATEGORIAN3, X.DTAULTVENDA, X.DTAULTENTRADA, X.ESTQLOJA, MARCA, COMPRADOR, G.SEQPESSOA, G.NOMERAZAO
