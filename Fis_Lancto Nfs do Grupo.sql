...
SELECT DISTINCT X.NUMERONF, X.SEQPESSOA||' - '||(SELECT NOMERAZAO FROM CONSINCO.GE_PESSOA Y WHERE Y.SEQPESSOA = X.SEQPESSOA) FORNECEDOR,
       TO_CHAR(X.DTAEMISSAO, 'DD/MM/YYYY') DTAEMISSAO, TO_CHAR(X.DTAENTRADA, 'DD/MM/YYYY') DTAENTRADA, X.DTAENTRADA - X.DTAEMISSAO DIF_DIAS, X.CODGERALOPER CGO, 
...
WHERE X.SEQPESSOA < 999 
  ...
  AND X.DTAENTRADA > SYSDATE - 2 --BETWEEN :DT1 AND :DT2
  AND X.NROEMPRESA = 1 --IN (#LS1) 
