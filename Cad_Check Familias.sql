...
SELECT DISTINCT X.NROEMPRESA EMPRESA, M.SEQFAMILIA, X.SEQPRODUTO PLU, 
       ...
       (SELECT MAX( USUAUDITORIA) FROM MAP_AUDITORIA Z WHERE (REGEXP_SUBSTR (VLRATUAL, '(\S*)')) = M.SEQFAMILIA AND DESCCAMPO = 'SEQFAMILIA'
       AND DTAHORAUDITORIA BETWEEN :DT1 AND :DT2 ) USUALTERACAO,
       (SELECT TO_CHAR(MAX(DTAAUDITORIA), 'DD/MM/YYYY')  FROM MAP_AUDITORIA Z WHERE (REGEXP_SUBSTR (VLRATUAL, '(\S*)')) = M.SEQFAMILIA AND DESCCAMPO = 'SEQFAMILIA'
       AND DTAHORAUDITORIA BETWEEN :DT1 AND :DT2 ) DTAALTERACAO
...
FROM MRL_PRODEMPSEG X LEFT JOIN MON_SEGMENTO Z ON X.NROSEGMENTO = Z.NROSEGMENTO
 ...
 SELECT SEQPRODUTO FROM MAP_PRODUTO WHERE SEQFAMILIA IN (
        SELECT DISTINCT (REGEXP_SUBSTR (VLRATUAL, '(\S*)')) FROM MAP_AUDITORIA X 
                  WHERE CAMPO = 'SEQFAMILIA' AND DESCCAMPO = 'SEQFAMILIA' AND DTAHORAUDITORIA BETWEEN :DT1 AND :DT2 ))
    AND QTDEMBALAGEM = 1
