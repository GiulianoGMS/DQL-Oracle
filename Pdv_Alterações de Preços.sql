-- QRP https://github.com/Giulianogms/QRPs/blob/main/RelAltPrecos.QRP

SELECT A.SEQPRODUTO, B.CODACESSO EAN, A.DESCCOMPLETA DESCRICAO, TRIM(A.QTDEMBALAGEM) QTDEMBALAGEM,
       NVL(CONSINCO.FMAD_PRECOPROGDATA(A.SEQPRODUTO, A.QTDEMBALAGEM, A.NROSEGMENTO, A.NROEMPRESA, (SELECT MAX(DTAPROGALTERACAO) FROM CONSINCO.MAD_PRODLOGPRECO AA WHERE
                                                                     AA.NROEMPRESA = A.NROEMPRESA AND AA.NROSEGMENTO = A.NROSEGMENTO AND AA.QTDEMBALAGEM = A.QTDEMBALAGEM
                                                                 AND AA.SEQPRODUTO = A.SEQPRODUTO) -1), 0) PRECO_ANTERIOR,
       ...
       'De: '||'#LT1'||' Até: '||'#LT2' PERIODO,
       :vnc5NroEmpresa EMPRESA

  FROM MRLV_BASEETIQUETAPROD A LEFT JOIN CONSINCO.MAP_PRODCODIGO B ON A.SEQPRODUTO = B.SEQPRODUTO AND B.TIPCODIGO = 'E'
                               LEFT JOIN CONSINCO.DIM_CATEGORIA  C ON C.SEQFAMILIA = A.SEQFAMILIA
  
 WHERE NROEMPRESA  IN (:vnc5NroEmpresa)                               
   AND NROSEGMENTO IN (SELECT X.NROSEGMENTOPRINC FROM MAX_EMPRESA X WHERE X.NROEMPRESA IN (:vnc5NroEmpresa))              
   ...
   AND NVL(CONSINCO.FMAD_PRECOPROGDATA(A.SEQPRODUTO, A.QTDEMBALAGEM, A.NROSEGMENTO, A.NROEMPRESA, (SELECT MAX(DTAPROGALTERACAO) FROM CONSINCO.MAD_PRODLOGPRECO AA WHERE
                                                                     AA.NROEMPRESA = A.NROEMPRESA AND AA.NROSEGMENTO = A.NROSEGMENTO AND AA.QTDEMBALAGEM = A.QTDEMBALAGEM
                                                                 AND AA.SEQPRODUTO = A.SEQPRODUTO) -1), 0) != DECODE(PRECOVALIDPROMOC,'0',PRECOVALIDNORMAL,PRECOVALIDPROMOC)
                                                                 
                                                                 ORDER BY CATEGORIAN1,3
