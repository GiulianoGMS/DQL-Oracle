...
SELECT XX1.*,
 /*Select de Subtototais*/
  ...
  FROM FI_TITULO X 
 WHERE X.NROTITULO    IN (#NR1) AND NROEMPRESA = 502 AND SITUACAO != 'C') TOT_TIT_GER,
 /******/
 ...
 WHERE UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 1)) FP FROM dual)||'%')
    OR UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 2)) FP FROM dual)||'%')) TOT_ORC_GER,
 /******/
 /*Diferenca*/
'R$ '||TO_CHAR(((SELECT SUM(VLRORIGINAL)
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA
 WHERE X.NROTITULO    IN (#NR1) AND NROEMPRESA = 502 AND SITUACAO != 'C')  -
 /******/
 ...
 /******/
  ...
  FROM CONSINCO.OR_NFDESPESA ZZ INNER JOIN CONSINCO.OR_NFVENCIMENTO ZX ON ZX.SEQNOTA = ZZ.SEQNOTA       
 WHERE UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 1)) FP FROM dual)||'%'))),  'FM999G999G990D90', 'NLS_NUMERIC_CHARACTERS='',.''') DIFERENCA,
...
SELECT * FROM (
 SELECT 'TITULO' TIPO, CODESPECIE, SEQTITULO, NROEMPRESA EMP, X.SEQPESSOA||' - '||NOMERAZAO FORNECEDOR, NROTITULO, #NR2 PROCESSO,
  ...
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA
 WHERE X.NROTITULO IN (#NR1) AND NROEMPRESA = 502 AND SITUACAO != 'C' AND X.OBRIGDIREITO = 'D' ORDER BY 9)
...
SELECT 'ORCAMENTO', NULL ,NULL, NULL, 'TOTAL NFS DESP NACIONAIS', NULL, NULL,
 ...
 WHERE UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 1)) FP FROM dual)||'%')
    OR UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 2)) FP FROM dual)||'%')
