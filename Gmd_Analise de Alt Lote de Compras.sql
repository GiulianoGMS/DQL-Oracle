...
SELECT X.SEQGERCOMPRA NROLOTE, COMPRADOR, DESCRITIVO, 
       FO.SEQFORNECEDOR COD_FORNEC, NOMERAZAO FORNEC,
       TO_CHAR(X.DTAHORINCLUSAO, 'DD/MM/YYYY HH24:MI') DTAINCLUSAO, X.USUINCLUSAO, 
       ...
       NVL(NULLIF(QTDPEDIDA,0), XI.QTDTRANSF) QTD_UNI_PEDIDA, 
       CASE WHEN NVL(NULLIF(QTDPEDIDA,0), XI.QTDTRANSF) = 0 
         THEN NULL ELSE QTDEMBALAGEM END QTDEMBALAGEM, ROUND((TFVLRCUSTOBRUTO) * NVL(NULLIF(QTDPEDIDA,0), XI.QTDTRANSF),2) VALOR,
       CASE WHEN NVL(NULLIF(QTDPEDIDA,0), XI.QTDTRANSF) = 0
