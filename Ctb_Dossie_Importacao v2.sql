...
SELECT XX1.*,
 /*Select de Subtototais*/
  ...
  FROM FI_TITULO X 
 WHERE X.NROTITULO    IN (#LT2) AND NROEMPRESA = 502 AND SITUACAO != 'C') TOT_TIT_GER,
 /******/
 ...
 WHERE ZZ.DTAEMISSAO > SYSDATE - 365
   AND (UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 1)) FP FROM dual)||'%')
    OR UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT CASE WHEN TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 2)) IS NOT NULL 
...
'R$ '||TO_CHAR(((SELECT SUM(VLRORIGINAL)
 ...
 WHERE X.NROTITULO    IN (#LT2) AND NROEMPRESA = 502 AND SITUACAO != 'C')  -
 /******/
...
SELECT * FROM (
 SELECT 'TITULO' TIPO, CODESPECIE, SEQTITULO, NROEMPRESA EMP, X.SEQPESSOA||' - '||NOMERAZAO FORNECEDOR, NROTITULO, #NR2 PROCESSO,
  ...
  FROM FI_TITULO X INNER JOIN GE_PESSOA Z ON X.SEQPESSOA = Z.SEQPESSOA
 WHERE X.NROTITULO IN (#LT2) AND NROEMPRESA = 502 AND SITUACAO != 'C' AND X.OBRIGDIREITO = 'D' ORDER BY 9)
...
SELECT 'ORCAMENTO' TIPO, CG.CODESPECIE CODESPECIE,
 ...
 WHERE ZZ.DTAEMISSAO > DATE '2024-01-01'
   AND (UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 1)) FP FROM dual)||'%')
    OR UPPER(ZZ.OBSERVACAO) LIKE UPPER('%'||(SELECT CASE WHEN TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 2)) IS NOT NULL 
                                                         THEN TRIM(REGEXP_SUBSTR('#LT1', '[^,]+', 1, 2)) ELSE 'X_GLN' END FP FROM dual)||'%'))
...
SELECT 'NF' TIPO, CODESPECIE, SEQTITULO, NROEMPRESA EMP, X.SEQPESSOA||' - '||NOMERAZAO FORNECEDOR, NROTITULO, #NR2 PROCESSO,
 ...
 WHERE X.NROTITULO IN (SELECT C.NUMERONF FROM MLF_NOTAFISCAL C
   ...
   AND NROEMPRESA = 503 AND X.SITUACAO NOT IN ('C')
   AND UF = 'EX'
