...
SELECT DISTINCT
      ...
      (SELECT LISTAGG(CODACESSO, ' ') WITHIN GROUP(ORDER BY SEQPRODUTO)
       ...
       NOMERAZAO FORNEC,
       CASE WHEN EXISTS (SELECT 1 FROM TMP_LOYALT_PRODACELERADORES ACE WHERE ACE.SEQPRODUTO = XI.SEQPRODUTO) THEN 'Sim' ELSE 'Não' END ACELERADOR
      

  FROM MRL_ENCARTE X INNER JOIN MRL_ENCARTEPRODUTO XI         ON XI.SEQENCARTE = X.SEQENCARTE
                     INNER JOIN MRL_ENCARTEPRODUTOPRECO PP    ON PP.SEQENCARTE = X.SEQENCARTE AND PP.SEQPRODUTO = XI.SEQPRODUTO
