...
SELECT DISTINCT 'Liberado por Senha' SITUACAO, TO_CHAR(DTAENTRADA, 'DD/MM/YYYY') DTAENTRADA, X.NROEMPRESA, X.NUMERONF, NOMERAZAO FORNEC, XI.SEQPRODUTO, DESCCOMPLETA DESC_PRODUTO, 
       TO_NUMBER(PR.QTDEMBALAGEM) EMB_PED,PI.QTDRECEBIDA QTD_PED,
       TO_NUMBER(XI.QTDEMBALAGEM) EMB_NF, XI.QUANTIDADE  QTD_NF,
       ROUND(((XI.QUANTIDADE - PI.QTDRECEBIDA) / QTDRECEBIDA) * 100,2) PERCENTUAL_DIF,
...
SELECT DISTINCT 'Pedido Adicionado' SITUACAO, TO_CHAR(DTAENTRADA, 'DD/MM/YYYY') DTAENTRADA, X.NROEMPRESA, X.NUMERONF, NOMERAZAO FORNEC, XI.SEQPRODUTO, DESCCOMPLETA DESC_PRODUTO, 
       TO_NUMBER(PR.QTDEMBALAGEM) EMB_PED,PI.QTDRECEBIDA QTD_PED,
       TO_NUMBER(XI.QTDEMBALAGEM) EMB_NF, XI.QUANTIDADE  QTD_NF,
       ROUND(((XI.QUANTIDADE - PI.QTDRECEBIDA) / QTDRECEBIDA) * 100,2) PERCENTUAL_DIF,
