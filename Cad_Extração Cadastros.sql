...
SELECT DISTINCT A.SEQPRODUTO PLU, B.CODACESSO EAN, A.DESCCOMPLETA NOME,F.MARCA,
                ...
                TO_CHAR(MAX(II.DTAULTCOMPRA),    'DD-MM-YYYY')          DATA_ULT_COMPRA,
                TO_CHAR(MAX(A2.DTAHORFECHAMENTO),'DD-MM-YYYY')          DATA_ULT_PEDIDO,
                TO_CHAR(MAX(II.DTAULTENTRADA),   'DD-MM-YYYY')          DATA_ULT_ENTRADA,
...
FROM MAP_PRODUTO A
       LEFT JOIN CONSINCO.MAP_PRODCODIGO B      ON A.SEQPRODUTO  = B.SEQPRODUTO AND B.TIPCODIGO = 'E' AND B.QTDEMBALAGEM = 1
       ...
       LEFT JOIN CONSINCO.MAC_GERCOMPRA BB      ON AA.SEQGERCOMPRA = BB.SEQGERCOMPRA WHERE BB.SITUACAOLOTE = 'F' AND BB.TIPOLOTE IN ('E', 'T')) A2 ON A2.SEQPRODUTO = A.SEQPRODUTO
       LEFT JOIN QLV_CATEGORIA@CONSINCODW QL    ON QL.SEQFAMILIA = A.SEQFAMILIA

GROUP BY A.SEQPRODUTO, B.CODACESSO, 
         A.DESCCOMPLETA, MARCA, MF.QTDEMBALAGEM, MF.EMBALAGEM, MF.QTDUNIDEMB, MF.FATORCONVEMBNFE, MF.STATUS, QL.APELIDO
