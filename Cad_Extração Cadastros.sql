SELECT DISTINCT A.SEQPRODUTO PLU, 
                ...
                MF.QTDEMBALAGEM QUANTIDADE, MF.EMBALAGEM, MF.QTDUNIDEMB QTDE_UNID_EMB, MF.FATORCONVEMBNFE FATOR_CONV_EMB_NFE, MF.STATUS, 
               ...
               (SELECT TO_CHAR(MAX(BB.DTAHORFECHAMENTO), 'DD/MM/YYYY') FROM CONSINCO.MAC_GERCOMPRAITEM AA 
                INNER JOIN CONSINCO.MAC_GERCOMPRA BB     ON AA.SEQGERCOMPRA = BB.SEQGERCOMPRA WHERE BB.SITUACAOLOTE = 'F' AND BB.TIPOLOTE IN ('E', 'T') AND AA.SEQPRODUTO = A.SEQPRODUTO) DATA_ULT_PEDIDO,
             ...
             --  (SELECT SUM(QTDOPERACAO) FROM FATOG_VENDADIA@CONSINCODW X WHERE X.SEQPRODUTO = A.SEQPRODUTO AND CODGERALOPER IN (37,48,123,610,615,613,810,916,910,911) AND DTAOPERACAO > SYSDATE - 180) VDA_ULTIMO_SEMESTRE
               (SELECT SUM(X.QTDVDA) FROM CONSINCO.MRL_CUSTODIA X WHERE X.SEQPRODUTO = A.SEQPRODUTO AND X.DTAENTRADASAIDA > SYSDATE - 180) VDA_ULTIMO_SEMESTRE,
                QL.APELIDO COMPRADOR, QL.CATEGORIA_NIVEL_1, QL.CATEGORIA_NIVEL_2, QL.CATEGORIA_NIVEL_3, QL.CATEGORIA_NIVEL_4, QL.CATEGORIA_NIVEL_5
...
FROM MAP_PRODUTO A
       ...
       WHERE QL.CATEGORIA_NIVEL_1 = 'BAKE SHOP'
        
--WHERE A.SEQPRODUTO NOT IN (SELECT DISTINCT SEQPRODUTO FROM CONSINCO.DIM_PRODUTOEMPRESA Z WHERE Z.STATUSCOMPRA = 'I')

GROUP BY A.SEQPRODUTO, A.DESCCOMPLETA, MARCA, MF.QTDEMBALAGEM, 
         MF.EMBALAGEM, MF.QTDUNIDEMB, MF.FATORCONVEMBNFE, MF.STATUS, QL.APELIDO,
