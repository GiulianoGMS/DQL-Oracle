...
FROM CONSINCO.FI_TITULO F 
                 ...
                 AND DDA.NRODOCUMENTO LIKE ('%'||F.NRODOCUMENTO||'%')
                    LEFT JOIN CONSINCO.FIV_DDATITULOSBUSCA DDA2 ON /* Validadores dos títulos não conciliados */
                 ...
                 OR GE.NOMERAZAO LIKE ('%'||DDA2.DESCFORNECEDOR||'%') AND ((F.VLRORIGINAL - (FC.VLRDESCCONTRATO + F.VLRPAGO)) = (DDA2.VALORDOCUMENTO - (DDA2.VALORDESCONTO1 + DDA2.VALORABATIMENTO))) AND (LPAD(DDA2.NROCNPJCPFSACADO,12,0)||LPAD(DDA2.DIGCNPJCPFSACADO,2,0) IN (SELECT LPAD(DE.NROCGCCPF,12,0)||LPAD(DE.DIGCGCCPF,2,0) FROM CONSINCO.GE_PESSOA DE WHERE SEQPESSOA IN (2)))
